...
void showHelp(void)
{
	...
	printf("  -a        = Stay running with automatic upload when JED/BIT is modified\n");
	printf("  -q <title>= Quit app where window title contains comma separated strings\n");
	printf("  -r <path> = Restart app after operation\n");
...
int processXCF(char *xcf, char *vme)
{
	...
	if ((xf = fopen(xcf, "rt")) != NULL)
	{
		...
		{
			tmp_str[0] = '\0';
			fgets(tmp_str, sizeof (tmp_str) -1, xf);
...
#error linux
	...
	if ((cf = fopen(config_file, "wt")) != NULL)
	{
		...
		fprintf(cf, "# -s\n");
		fprintf(cf, "# -f\n");
		fprintf(cf, "# -c C:\\Program Files\\lscc\\Programmer\\2.1_x64\\bin\\nt64\\ddtcmd.exe\n");
		...
		for (;;)
		{
			...
			if (str[0] == '-')
			{
				switch(str[1])
				{
					case 'a':
						gAutomatic = 1;
						printf("Automatic monitoring mode enabled (process when JED/BIT file changes)\n");
						break;
					case 'v':
						gVerbose = 1;
					...
					case 'q':
						...
						strncpy(gKillTitleStrs, &str[3], sizeof (gKillTitleStrs)-2);
						break;
					case 'r':
						memset(gRestartPath, 0, sizeof(gRestartPath));
						strncpy(gRestartPath, &str[3], sizeof (gRestartPath)-1);
						break;
					default:
						SetRedText();
...
//	OpenLine(2);
	...
	if ( rc >= 0 )
	{
		...
		while (gAutomatic)
		{
			...
			{
				SetYellowText();
				printf("\nNOTE: Automatic mode requires use of XCF file.\n");
				SetNormalText();
				exit(0);
			}

			memset(&o_stat, 0, sizeof (o_stat));
			memset(&n_stat, 0, sizeof (n_stat));
			memset(&nn_stat, 0, sizeof (nn_stat));

			SetYellowText();
			printf("\nAutomatic mode, monitoring file:   (Close window or Ctrl-Break to cancel)");
			SetNormalText();
			printf("\n  \"");
			SetBoldText();
			printf("%s", fpga_file);
			...
			if (stat(fpga_file, &o_stat) != 0)
			{
				printf("\nCan't get file stats for \"%s\", quitting.\n", fpga_file);
				gRetCode = -1;
				exit(gRetCode);
			}
			gRetCode = 0xc0de1;
			while (1)
			{
				printf("\b%c", twid[twiddle]);
				fflush(stdout);
				if (++twiddle >= strlen(twid))
					twiddle = 0;
				
				Sleep(AUTO_POLL_DELAY_MS);
				
				if (stat(fpga_file, &n_stat) == 0)
				{
					if (o_stat.st_mtime != n_stat.st_mtime || o_stat.st_size != n_stat.st_size)
					{
						printf("\b*");
						fflush(stdout);
						Sleep(1000);
						if (stat(fpga_file, &nn_stat) == 0)
						{
							if ((nn_stat.st_mtime != n_stat.st_mtime) || (nn_stat.st_size != n_stat.st_size))
								continue;
						}
						
						printf("\nModification detected, reprocessing.\n");
						fflush(stdout);
						if (processXCF(xcf_file, vme_file) != 0)
						{
							SetRedText();
							printf("Error: XCF processing failed, retrying.\n");
							SetNormalText();
						}
						else
							ready = 1;
